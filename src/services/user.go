...
func GenCert(genCertReq *models.GenCertReq) ([]byte, error) {
	...
	//检查完参数看看证书是否存在
	certContent, err := models.FindCertContentByConditions(genCertReq.UserID, genCertReq.OrgID, genCertReq.CertUsage, genCertReq.UserType)
	if err == nil {
		err = fmt.Errorf("cert is existed")
		logger.Error("Generate Cert", zap.Error(err))
		reCertContent, _ := base64.StdEncoding.DecodeString(certContent.Content)
		return reCertContent, err
	}
	//先去生成csr流文件
